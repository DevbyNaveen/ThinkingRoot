...
/// for an atomic fact: a short situating prefix (document title + the chunk's
/// one-line memory brief) followed by the verbatim statement. Only the embedded
/// text changes; the stored statement / vector key / meta stay verbatim, so a
/// retrieved fact is still the exact quote. Empty pieces are skipped; with
/// neither title nor brief it degrades to the bare statement (no regression).
