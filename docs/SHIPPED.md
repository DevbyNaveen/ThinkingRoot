...
| **34. SOTA agent loop UX — Continue?, retry, parallel reads, live progress, inline diffs** — Closes the user-reported "agent stopped at iteration ceiling" failure cluster and brings the desktop chat surface to Cursor / Claude-Code parity. **Backend (`agent.rs`):** `max_iterations` default 8 → 25; LLM call wrapped in exponential-backoff retry (`LLM_RETRY_ATTEMPTS = 3`, `llm_retry_backoff(1)=1s / (2)=2s / (3)=4s`) with cancellation-aware `tokio::select!` and `Error::is_permanent` short-circuit (401/403/400 skip retry); `DEFAULT_TOOL_RESULT_TOKEN_BUDGET` 2048 → 8_192 (`token_budget.rs:37`); new `AgentEvent::ContinuationOffered { partial_text, iterations_used, reason }` variant replaces 3 fatal paths (`iteration_budget` / `max_tokens` / `loop_detected`) followed by terminal `Done` so chat-persistence + Observer hooks still run on partial reply; new `AgentEvent::ToolCallProgress { id, name, partial_content, byte_count }` variant; parallel-read dispatch refactored to emit `ToolCallExecuting` in proposal order BEFORE `FuturesUnordered` dispatch starts (pre-fix flickered: "read[3] running before read[1] executing"); `Vec<Option<ToolResult>>` slot-fill keeps history shape balanced. SSE + trace wire mappings extended for both new variants (`AGENT_CONTINUATION_OFFERED`, `AGENT_TOOL_PROGRESS`); exhaustive `event_to_trace_covers_every_agent_event_variant` test forces every future variant to be mapped. **UI (`ChatView.tsx`, `ClaimCard.tsx`):** chronological card interleaving via `buildStreamParts(streaming)` — each `tool_call_proposed` snapshots `streaming.partial.length` into new `AgentStep.proposedAtPartialLen`; renderer sorts by that offset + splits the prose at each step's snapshot to slot cards inline (Cursor-style, never above prose). New `ContinuationPrompt` amber banner with reason-aware copy (`iteration_budget`→"Reached the step budget", `max_tokens`→"Output was cut off", `loop_detected`→"Stuck on the same approach") + Continue / Stop-here buttons — replaces dead-end red error. New `ThoughtBlock` for `think` tool (collapsed "Thought ▸" with Lightbulb icon, rules-of-hooks compliant early-return after hook calls). New `WriteToolInputView` (amber-highlighted structured `key: value` rows for writes, monospace fallback for reads). Read tools collapse to status line; writes auto-expand. Live `step.progress` rendering with blinking caret + `humanBytes(step.progressBytes)`. New store actions `appendStepProgress` + `setContinuationOffer`; `ChatEvent` union extended with `tool_call_progress` + `continuation_offered`. **Not in this ship (deferred):** `ToolCallProgress` emitters for `compile` / `shell_exec` (event + wire + UI ready; per-tool migration is a focused ship); citation chips with `content_blake3` verify badge (`[claim:<id>]` markers emitted; ChatMarkdown chip parser refactor pending). | 2026-05-18 | ✅ shipped (commit `853660d`) | thinkingroot-serve lib 802 passing (0 regressions); UI `pnpm tsc --noEmit` clean; `cargo check --workspace` clean (1 pre-existing dead_code warning) |
| **33. SOTA prompt foundation — env injection, agentmemory recall, MCP telemetry reminders, `think` tool, structured errors, skill auto-surface, parallel reads** — Six load-bearing changes that bring the in-app agent's prompt assembly + tool-use surface to Claude-Code parity (informed by audit of `~/Desktop/src/` + 2026 SOTA research: Anthropic context engineering, Agent Skills tiered disclosure, `think` tool, Mem0 cross-session memory, parallel tool dispatch). **(1) Environment block** (new `intelligence/environment.rs`, 285 LOC, 7 unit tests) — `EnvironmentInfo::gather()` captures cwd, `$HOME`, `~/Desktop`, `~/Documents`, `~/Downloads`, OS, shell, `today_iso`; `render_block` emits a `<system-reminder># environment` block honestly-skipping None fields; byte-deterministic across calls (pinned). Kills the "AI can't resolve Desktop without `ls`" failure class. **(2) 11-block reminder bus stable order** (`reminder_bus.rs`, 1107 LOC, 33 tests incl. `full_v2_context_renders_all_eleven_blocks_in_stable_order`) — `environment → workspace → agentmemory_recall → relevant_skill → branch_state → session_state → engram_state → mcp_sessions → substrate_health → sandbox_alert → tool_budget`. Stable→volatile ordering keeps the Anthropic auto-prompt-cache breakpoint at the top of the volatile zone. New slim projection types: `AgentmemoryRecall`, `McpSessionBrief`, `RecoveryEventBrief`, `RelevantSkill`. **(3) Cross-session agentmemory recall** (`rest.rs::agent_stream_response` + `recovery_event_brief` projection) — Mem0/Letta top-K auto-surface: `engine.search` on the user's message before agent-loop start; top-3 relevant claims inject as a reminder block. **(4) SOTA_OPERATING_PRINCIPLES** (`synthesizer.rs:421`, ~750 tokens) — appended in `compose_full` between skills manifest and persona operator section. Seven topics: honesty in-band, action safety/reversibility tiers, tool-use principles (parallel reads + bounded retry + structured error envelopes), witness-pinned citations, `think` tool guidance, prompt-injection awareness, URL safeguard, tool-grounded verification. **Memory persona excluded** (`compose_full_memory_persona_does_not_get_sota_principles`) to preserve LongMemEval byte-pin. **(5) `think` tool** (`builtin_tools.rs::ThinkTool`, 3 tests) — no-op `{thought}→"noted: …"` echo; read-class; UI renders as collapsed Thought ▸ block. Anthropic-published 2025 pattern, +54% policy adherence on τ-bench. **(6) Structured-error envelope** (`tools.rs::ToolHandlerResult::structured_error(error_type, hint, retryable)`, 2 tests) — `{ok:false,error_type,hint,retryable}` JSON; LLM is told how to read it in `SOTA_OPERATING_PRINCIPLES`. **Plus:** `SkillRegistry::pick_relevant(query)` keyword-overlap classifier (sub-µs, stop-word filtered, `SKILL_MATCH_MIN_SCORE=2`, lex-tiebreak; 8 tests); parallel-read dispatch substrate (preparation for Ship 34's flicker fix). | 2026-05-18 | ✅ shipped (commit `31a5985`) | thinkingroot-serve lib 774 → 802 (+28 all green); synthesizer prompt_contract 33 → 35; integration suite 968 unchanged; `cargo check --workspace` clean (1 pre-existing dead_code warning) |
| **32. SOTA reranker swap + install-time model bundle (Phase A)** — Replaces fastembed v5 (HF-Hub lazy fetch, incompatible with the bundle-at-install contract) with direct `ort 2.0.0-rc.11` + `tokenizers 0.22.2` plumbing. **New module** `crates/thinkingroot-graph/src/ort_session.rs` (~700 LOC): `EmbeddingModel::load` (BERT-family with optional `token_type_ids`, mean-pool + L2-normalise) + `CrossEncoderModel::load` (pair-encode, sigmoid-normalised logits), both from explicit `OrtModelPaths` with loud-fail-on-missing + `root doctor --fix` hint. Output-name detection picks `last_hidden_state` / `logits` if present, falls back to first declared output with a `tracing::warn!` so the operator sees the substitution. **`default_model_bundle_dir()`** resolves the canonical bundle dir (`<dirs::cache_dir>/thinkingroot/models/`) with `THINKINGROOT_MODELS_DIR` env override for tests + portable installs. **Rewrites** `vector.rs` (AllMiniLM-L6-v2, 384-dim, EMBED_DIM pinned because `vectors.bin` schema bakes it in — index stays compatible across the swap) and `rerank.rs` (now `gte-reranker-modernbert-base`, ~300 MB FP16, 8192-tok context, 83% Hit@1 on independent benchmarks — replaces the Jina Turbo wiring that Track 26's commit body explicitly flagged as a deferred swap). **`ScoringProfile::default()` flips `use_cross_encoder = true`** for in-process construction; serde-default stays `false` so v0.9.x daemons receiving v0.9.y request payloads that omit the field get back-compat behaviour (no silent upgrade). New `ScoringProfile::instant()` preset for typeahead (`<25 ms` budget paths) explicitly opts back out. **Phase B installer pipeline:** new `ModelBundle` + `ModelFile` types in `crates/thinkingroot-core/src/install_manifest.rs` with `#[serde(default)]` for back-compat (existing v1 manifests in the wild deserialize cleanly as `model_bundle: None`); `ModelFile::verify()` stream-hashes both files against recorded BLAKE3 anchors. New `models.bundle_present` doctor check (12th stable check ID, commit-locked): Skipped when no manifest, Fail when `model_bundle: None` or files missing, Warn on BLAKE3 mismatch, Ok when bundle verifies. Fix command: re-run installer. **`install.sh`** gains `MODELS_TAG="models-v1"`, `TR_SKIP_MODELS` opt-out, `model_bundle_dir()` helper, `install_model_bundle()` that downloads 4 files + verifies each against the pinned SHA-256 from `SHA256SUMS` (rejects tampered downloads), computes BLAKE3 via `root hash-file`, sets 9 shell globals that `write_install_manifest` emits into the `model_bundle` JSON block. Idempotent — already-cached files with matching SHA-256 skip the network fetch. **`install.ps1`** mirror: `Install-ModelBundle` function with `$script:ModelBundle*` globals + JSON emission via `[pscustomobject]`. **Test scoreboard delta:** thinkingroot-graph 134 → 138 (+4 ort_session); thinkingroot-cli 35 → 39 (+4 models_bundle_present); core / serve unchanged. **What's NOT in this ship:** GitHub Release `models-v1` tag itself (one-time admin step: mirror AllMiniLM-L6-v2 + gte-reranker-modernbert-base ONNX + tokenizer files from HF to GitHub Releases with a `SHA256SUMS` file); warm-on-boot in `serve.rs` (Phase C — hides 2-3 s cold start behind EngineGate's "booting" state); the projected 95-96% LongMemEval lift, which requires Tracks 33 (OMEGA-style query expansion) + 34 (Observer/Reflector category tuning) on top of this swap. | 2026-05-16 | ✅ shipped (Phase A + B) | +4 ort_session graph + +4 models_bundle_present cli; 503 serve unchanged; whole-workspace `cargo check` clean |
| **31. Legacy grounding + NLI ONNX cleanup — 175 MB blob purge + dead-dep prune** — Post-Witness-Mesh audit (2026-05-15) confirmed: the `thinkingroot-ground` crate had ZERO runtime consumers anywhere in the workspace (no `use thinkingroot_ground::` outside the crate itself). Yet `install.sh:189` + `install.ps1:141` still downloaded a ~83 MB NLI ONNX model on every fresh install, and `crates/thinkingroot-ground/models/` carried **175 MB of checked-in ONNX models + tokenizer.json** that no production code path read. Honesty rule §7 (verify before recommending) said the installer was selling a model the engine deleted on 2026-05-11. **Cuts:** (1) `crates/thinkingroot-ground/models/` entirely (87 MB × 2 ONNX + 8 MB tokenizer = 175 MB → 16 KB); (2) `lexical.rs` + `LexicalJudge` re-export + `unicode-segmentation` dep (last consumer was the deleted Rooting crate); (3) `install.sh` lost `NLI_MODELS_TAG`, `nli_onnx_filename`, `install_nli_models`, `model_cache_dir`, the `TR_SKIP_NLI` doc + call-site, ~50 LOC total; (4) `install.ps1` lost `$NLI_MODELS_TAG`, `$NliOnnx`, `$ModelDir`, `Install-NliModel`, the `TR_SKIP_NLI` doc + call-site, ~45 LOC total; (5) `thinkingroot-ground` workspace-dep dropped from `thinkingroot-cli/Cargo.toml` + `thinkingroot-serve/Cargo.toml` (declared but never imported). **Surviving:** `witness_verifier.rs` (8 unit tests, the canonical mechanical BLAKE3 anchor check that `tr-verify` will call into) + its module-level docstring updated to record the 2026-05-15 cleanup alongside the 2026-05-11 cutover. **Why no migration script:** the NLI model lived under `~/Library/Caches/thinkingroot/models/` (macOS) or `~/.cache/thinkingroot/models/` (Linux) or `%LOCALAPPDATA%/thinkingroot/models` (Windows) — cache directories, safe to leave; users running `rm -rf` on those paths reclaim disk without breaking anything. The next installer run won't repopulate them. **Honest model story is now:** retrieval uses exactly ONE model — `fastembed AllMiniLML6V2` (~30 MB, lazy-download on first compile via HF Hub, vector feature-gated); grounding uses zero models — `witness_verifier::verify_witness_anchor` is a BLAKE3 comparison (~10µs/witness) on byte-anchored Witness rows. | 2026-05-15 | ✅ shipped | 8 ground unit tests pass (was 11 — 3 lexical tests deleted with their source); whole-workspace `cargo check` clean; `du -sh` of `thinkingroot-ground/` drops 175 MB → 16 KB |
| **30. Playground File Manager — folders, moves, previews, trash, multi-select** — Closes the system-features gap that left Playground at ~50% coverage of "useful for students / developers / anyone." Adds a full file-manager surface accessible via a new **Files** tab inside `PlaygroundView` (between Paper and Chat). New Tauri command surface in `apps/thinkingroot-desktop/src-tauri/src/commands/playground_fs.rs` (~680 LOC, 6 unit tests pinning path-safety + extension classification): `playground_list_directory`, `playground_create_folder`, `playground_rename`, `playground_move`, `playground_trash`, `playground_list_trash`, `playground_restore`, `playground_empty_trash`, `playground_preview`. Every operation routes through `safe_path_within` — canonicalises the workspace root + candidate, rejects `Component::ParentDir / RootDir / Prefix`, verifies `starts_with` — defending against `../escape` and symlink traversal. Trash convention: items move into `<ws>/.thinkingroot/trash/<unix-ts>-<name>` so timestamps prevent collision with prior trashed copies + `.thinkingroot/` is already walker-ignored so trashed items never re-extract. Preview tiering: text / markdown / code under 1 MiB returns full UTF-8 content; image under 5 MiB returns base64 data-URL; audio / video / pdf / oversized return `kind="binary"` + absolute path for "Open externally" via the existing Tauri opener plugin (no asset-protocol security review needed). **UI:** new `FileManager.tsx` (~580 LOC) + `FilePreviewPanel.tsx` (~120 LOC). Features: breadcrumb navigation, single/cmd-click toggle, shift-click range select, Ctrl-A select-all, Del → trash, F2 → rename, Esc → clear selection, drag-drop file move between folders, drop-on-up-button to move-up-one-level, drop-on-folder to move-into, in-place rename input, new-folder input, restore + empty-trash actions, honest collision feedback (skipped_conflict count surfaced in toasts). Filesystem-as-substrate design: folders are real `<ws>/inbox/...` directories so the user's mental model matches Finder/Explorer + the engine's walker, never a virtual tag layer. | 2026-05-15 | ✅ shipped | +6 backend unit tests (path-safety, extension classification, rel-path formatting); `cargo check` clean; `pnpm tsc --noEmit` clean; `pnpm build` clean (2.94s, FileManager + FilePreviewPanel both in static-import graph) |
| **29. Phase 5 closure — count_stale/orphaned + B1/B2/B3 audit** — Completes the three-bucket Phase 5 cutover discovered post-Track-28: (B1) `count_stale_claims` and `count_orphaned_claims` now sum both `claims` and `witnesses` rows so post-Track-14 workspaces (where the compile pipeline writes only witnesses) get honest freshness + cascade-completeness signals. Both gain dedicated regression tests that build witness-only fixtures and pin the union semantics. Two new private helpers `count_query_with_cutoff` + `count_query_simple` factor the count+parse boilerplate so further union counters land in 3 lines. (B2) Audited `count_low_grounding_claims` + `count_claims_by_admission_tier` for honesty: turns out the admission-tier counter already had a Witness Mesh fallback (lines 3698-3704; witnesses count as `attested` when no claim-side tier data exists), and `count_low_grounding_claims` is honest because it returns 0 for fresh workspaces — the `grounding_score` column is dead post-Track-14, so 0 IS the right answer. Updated docstring to mark the legacy-substrate-only semantics explicitly. (B3) Audited the high-traffic `list_claims_branched` + `list_claims` + workspace cache hydration paths — discovered they ALL route through `GraphStore::get_all_claims_with_sources()` (the Phase 5.1a bridge), so the witness-union read path was already shipped. No code change needed for B3. **Net result**: every read surface in the graph crate is now substrate-honest. Post-Track-14 workspaces (witness-only) and pre-Track-14 workspaces (claims-only) and mixed workspaces (both write paths) all surface the same honest "knowledge units" total to every downstream consumer. | 2026-05-15 | ✅ shipped (Phase 5 closure) | +2 union-count regression tests (`count_stale_claims_includes_stale_witnesses`, `count_orphaned_claims_includes_orphan_witnesses`); 136 graph + 16 health + 488 serve + 12 compile = 652 tests pass; whole-workspace `cargo check` clean |
| **28. Phase 5 honesty fix — `get_counts` reports witness-union, not bare claims** — Post-Track-14 the compile pipeline writes only `witnesses` while the contribute path still writes `claims`; both ID spaces are disjoint (ULIDs vs. content-addressed BLAKE3) so a fresh post-Track-14 workspace has `count(claims) == 0` even though the substrate is full. Every downstream consumer of `GraphStore::get_counts()` — `root health` provenance scoring (`crates/thinkingroot-health/src/verifier.rs:30`), pipeline README synthesis (`pipeline.rs:1745`), engine brief generation (`engine.rs:2794`), the four call sites in `compile/compiler.rs` — was reading the dead column and reporting "0 claims" / 0% provenance / empty knowledge graph on a workspace full of witnesses. **Fake data per CLAUDE.md Honesty Rule §1.** The fix sums both substrates inside `get_counts` (`s, c + w, e`) so all callers transparently see the honest "knowledge units" total without API ripple. Variable name `claim_count` stays at call sites — the docstring on `get_counts` is the canonical source of truth on the post-Track-14 semantics. **Why this doesn't double-count:** witness ids are `BLAKE3(rule \|\| canonical_cbor(spans))` while claim ids are ULIDs — the spaces are disjoint by construction. Two rows describing the same byte range from different write paths (witness from compile + claim from agent contribute) ARE separate knowledge units in the substrate model and SHOULD count as 2. Health verifier's back-compat provenance branch (`if rooted == 0 && quarantined == 0 && rejected == 0 { if claims > 0 && sources > 0 { 1.0 } else { 0.0 } }`) now correctly fires for witness-only workspaces — content-addressed BLAKE3 substrate is maximum-trust by construction (no LLM extraction = no admission tier to weight) so 100% provenance is the honest answer. | 2026-05-15 | ✅ shipped | +1 regression test pinning witness-only count semantics; all 16 health verifier tests + 134 graph tests + 488 serve tests + 12 compile tests pass; whole-workspace `cargo check` clean |
| **27. SOTA Lever 3 wire-in — Observer auto-records every chat turn** — Closes the gap Track 26 named: `spawn_agent_run` in `crates/thinkingroot-serve/src/intelligence/agent_streaming.rs` now interposes a relay between the spawned `Agent::run_streaming` task and the caller's `mpsc::Sender<AgentEvent>`. The relay forwards every event verbatim (no wire-shape change for SSE consumers — desktop's `chat-event` Tauri channel sees identical bytes) AND captures `final_text` from the terminal `AgentEvent::Done`. On agent completion, the new `record_completed_turn` helper allocates the next chat-turn ordinal via the new `SessionContext::next_chat_turn` method, calls `observer.record_turn(ChatTurn { … })`, and — when `should_reflect()` trips — drives `QueryEngine::flush_observations(ws, session_id)` so condensed observations + reflection witnesses land in the substrate automatically. **Honest semantics:** only completed turns produce observations; cancellation / SSE disconnect / `AgentEvent::Error` return `None` from the relay's capture (no Done = no memory). **Two counters, two purposes:** new `SessionContext.chat_turn_count` is distinct from the existing `turn_count` (which counts `contribute` write calls) so the Observer's `turn_number` ordinal is meaningful even for read-only sessions that never write claims. **Best-effort:** all observer/flush errors `tracing::warn!`-logged, never propagated — the chat reply has already streamed; observer is a downstream consolidation layer and must not corrupt a successful turn's wire response. **What's tested:** new `relay_events` helper extracted for testability; 3 new tokio tests pin (a) verbatim event forwarding + Done capture, (b) `None` return on no-Done, (c) drain-on-disconnect so the agent task never blocks. Plus 2 new session-counter tests pinning monotonic increment and independence from `turn_count`. **What's still opt-in:** the `observe_turn` / `flush_observations` MCP tools (shipped in Track 26) remain available for clients that prefer explicit recording — they coexist with the auto-recording. | 2026-05-15 | ✅ shipped | +5 new unit tests (3 relay-events + 2 session-counter); 488 serve-lib tests passing; whole-workspace `cargo check` clean |
